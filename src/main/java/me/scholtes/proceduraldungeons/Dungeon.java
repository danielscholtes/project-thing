...
final class Dungeon {
    ...
    private void generateStructure(final Location location, final int x, final int y, final String room) {
        ...
        if (isValidRoom > 0) {
            if (room.equalsIgnoreCase("0_0")) {
                location.clone().add(x * SIZE, 0, y * SIZE).getBlock().setType(Material.LIME_WOOL);
            } else {
                location.clone().add(x * SIZE, 0, y * SIZE).getBlock().setType(Material.LAPIS_BLOCK);
            }
        ...
        location.clone().add(x * SIZE, 0, (y * SIZE) + 1).getBlock().setType(Material.AIR);
        location.clone().add(x * SIZE, 0, (y * SIZE) + 2).getBlock().setType(Material.AIR);
        location.clone().add((x * SIZE) + 1, 0, y * SIZE).getBlock().setType(Material.AIR);
        location.clone().add((x * SIZE) + 2, 0, y * SIZE).getBlock().setType(Material.AIR);
        location.clone().add(x * SIZE, 0, (y * SIZE) - 1).getBlock().setType(Material.AIR);
        location.clone().add(x * SIZE, 0, (y * SIZE) - 2).getBlock().setType(Material.AIR);
        location.clone().add((x * SIZE) - 1, 0, y * SIZE).getBlock().setType(Material.AIR);
        location.clone().add((x * SIZE) - 2, 0, y * SIZE).getBlock().setType(Material.AIR);
    ...
    private int setDirectionBlocks(final Location location, final Direction direction, final Direction opposite, final int x, final int y, final int incrementX, final int incrementY, final String room) {
        ...
        if (rooms.get(room).getRoomType().toString().contains(direction.toString()) && adjacentRoom != null && adjacentRoom.getRoomType().toString().contains(opposite.toString())) {
            location.clone().add((x * SIZE) + direction.getX1(), 0, (y * SIZE) + direction.getY1()).getBlock().setType(Material.REDSTONE_BLOCK);
            location.clone().add((x * SIZE) + direction.getX2(), 0, (y * SIZE) + direction.getY2()).getBlock().setType(Material.REDSTONE_BLOCK);
            return 1;
        } else {
            location.clone().add((x * SIZE) + direction.getX1(), 0, (y * SIZE) + direction.getY1()).getBlock().setType(Material.STONE);
            location.clone().add((x * SIZE) + direction.getX2(), 0, (y * SIZE) + direction.getY2()).getBlock().setType(Material.STONE);
            return 0;
        }
    }

    private void setOutskirts(final Location location, final int x, final int y, final Number incrementY) {
        location.clone().add((x * SIZE) - 2, 0, (y * SIZE) + incrementY.intValue()).getBlock().setType(Material.STONE);
        location.clone().add((x * SIZE) - 1, 0, (y * SIZE) + incrementY.intValue()).getBlock().setType(Material.STONE);
        location.clone().add((x * SIZE) + 1, 0, (y * SIZE) + incrementY.intValue()).getBlock().setType(Material.STONE);
        location.clone().add((x * SIZE) + 2, 0, (y * SIZE) + incrementY.intValue()).getBlock().setType(Material.STONE);
    }
}
